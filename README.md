...
- Algorithm Lab CLI: `ascent list` prints stable algorithm IDs; `ascent compare … --json -o ascent_compare.json` runs multiple ascent algorithms on the same activity and reports deltas vs a baseline; `ascent bench … -o ascent_bench.json` aggregates delta distributions across many activities for regression testing.
